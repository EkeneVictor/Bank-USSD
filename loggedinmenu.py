...
def logged_in_menu():
    ...
    while True:
        ...
        def check_acct_details():
            ...
            try:
                print \
                    (f'\033[1m\n\t+------------------------------+\n\t|      Account    Details      \n\t+------------------------------+\n\t| First name:{user_data[1]}              \n\t| Last name:{user_data[2]}           \n\t| Phone number:{user_data[3]}            \n\t| User name:{user_data[4]}             \n\t| Account number:{user_data[5]}              \n\t| BVN:{user_data[6]}              \n\t| PIN:{user_data[8]}              \n\t| Account Balance:${user_data[7]}              \n\t| Account Status:{user_data[9]}              \n\t| Account Type:{user_data[10]}              \033[0m\n\t+------------------------------+\n')
        ...
        def delete_account():
            ...
            time.sleep(2)
            affirm_del = input(
                f'\n\033[0m\033[91m{user_data[4]} are you sure you want to delete your account [Y/N]: \033[0m').capitalize()
            time.sleep(2)
            if affirm_del == 'Y':
                del_acc_query = "DELETE FROM bank_tbl WHERE user_name = %s AND PIN = %s"
                my_cur.execute(del_acc_query, (user_name, pin_))
                conn_obj.commit()
                ...
                if sender_balance:
                    if sender_balance >= amount:
                        if reciever and reciever[9] == 'Active':
                            ...
                            log_transaction_failed(user_name, recipient_account, amount_tr, description, "[RECIPIENT ACCOUNT SUSPENDED]")
        ...
        if cntn == '1':
            ...
            time.sleep(3)
            print("\n\t\033[3mLogging out ...\033[0m")
            time.sleep(3)
            print("\t\033[3mLogged out successfully.\033[0m\n\n")
            options_menu()
            break
        elif cntn == '9':
            time.sleep(0.5)
